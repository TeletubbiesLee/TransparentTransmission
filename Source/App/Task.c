...
int TCP_Client2Uart(void)
{
	...
	socketFd = TCP_NetConnect(g_ConfigFile[REMOTE_IP_ADDRESS_NUM].configString, g_ConfigFile[REMOTE_PORT_NUM].configData);
...
int TCP_Server2Uart(void)
{
	...
	socketFd = TCP_NetListen(g_ConfigFile[LOCAL_PORT_NUM].configData);
	uartFd = UartInit(g_ConfigFile[UART_DEVICE_NAME_NUM].configString, g_ConfigFile[UART_BANDRATE_NUM].configData);
...
int UDP2Uart(void)
{
	...
	socketFd = UDP_NetConnect(g_ConfigFile[LOCAL_PORT_NUM].configData);
	uartFd = UartInit(g_ConfigFile[UART_DEVICE_NAME_NUM].configString, g_ConfigFile[UART_BANDRATE_NUM].configData);
 ...
 * @param param 整型数组，第一个数存放写管道号，第二个数存放网络socket描述符指针
 * @return void
 */
static void TCPClientReceivePthread(void *param)
{
